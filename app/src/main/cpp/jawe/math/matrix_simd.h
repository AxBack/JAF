...
#define M33 15
        ...
        public:
            ...
            static float32x4_t loadRow(const Matrix& m, UINT row)
            {
                UINT index = row*4;
                return load(m, index, index+1, index+2, index+3);
            }

            static float32x4_t loadColumn(const Matrix& m, UINT col)
            {
                return load(m, col, col+4, col+8, col+12);
            }

            static float32x4_t load(const Matrix& m, UINT i1, UINT i2, UINT i3, UINT i4)
            {
                float v[] = {m.m_data[i1], m.m_data[i2], m.m_data[i3], m.m_data[i4]};
                return vld1q_f32(v);
            }

            static float32x4_t load(const Matrix& m, UINT i1, UINT i2, UINT i3)
            {
                float v[] = {m.m_data[i1], m.m_data[i2], m.m_data[i3], 0};
                return vld1q_f32(v);
            }

            //specific for non aarch64
            static float mulAndAdd(float32x4_t lhs, float32x4_t rhs)
            {
                float data[4];
                vst1q_f32(data, vmulq_f32(lhs, rhs));
            ...
            static Matrix multiply(const Matrix &left, const Matrix &right) {
                ...
                if(Utils::SIMD::ready)
                {
                    float32x4_t row0 = loadRow(right, 0);
                    float32x4_t row1 = loadRow(right, 1);
                    float32x4_t row2 = loadRow(right, 2);
                    float32x4_t row3 = loadRow(right, 3);

                    float32x4_t col0 = loadColumn(left, 0);
                    float32x4_t col1 = loadColumn(left, 1);
                    float32x4_t col2 = loadColumn(left, 2);
                    float32x4_t col3 = loadColumn(left, 3);

#if defined (__aarch64__)
                    ret[M00] = vaddvq_f32(vmulq_f32(row0, col0));
                    ret[M10] = vaddvq_f32(vmulq_f32(row0, col1));
                    ret[M20] = vaddvq_f32(vmulq_f32(row0, col2));
                    ret[M30] = vaddvq_f32(vmulq_f32(row0, col3));

                    ret[M01] = vaddvq_f32(vmulq_f32(row1, col0));
                    ret[M11] = vaddvq_f32(vmulq_f32(row1, col1));
                    ret[M21] = vaddvq_f32(vmulq_f32(row1, col2));
                    ret[M31] = vaddvq_f32(vmulq_f32(row1, col3));

                    ret[M02] = vaddvq_f32(vmulq_f32(row2, col0));
                    ret[M12] = vaddvq_f32(vmulq_f32(row2, col1));
                    ret[M22] = vaddvq_f32(vmulq_f32(row2, col2));
                    ret[M32] = vaddvq_f32(vmulq_f32(row2, col3));

                    ret[M03] = vaddvq_f32(vmulq_f32(row3, col0));
                    ret[M13] = vaddvq_f32(vmulq_f32(row3, col1));
                    ret[M23] = vaddvq_f32(vmulq_f32(row3, col2));
                    ret[M33] = vaddvq_f32(vmulq_f32(row3, col3));
#else
                    ret[M00] = mulAndAdd(row0, col0);
                    ret[M10] = mulAndAdd(row0, col1);
                    ret[M20] = mulAndAdd(row0, col2);
                    ret[M30] = mulAndAdd(row0, col3);

                    ret[M01] = mulAndAdd(row1, col0);
                    ret[M11] = mulAndAdd(row1, col1);
                    ret[M21] = mulAndAdd(row1, col2);
                    ret[M31] = mulAndAdd(row1, col3);

                    ret[M02] = mulAndAdd(row2, col0);
                    ret[M12] = mulAndAdd(row2, col1);
                    ret[M22] = mulAndAdd(row2, col2);
                    ret[M32] = mulAndAdd(row2, col3);

                    ret[M03] = mulAndAdd(row3, col0);
                    ret[M13] = mulAndAdd(row3, col1);
                    ret[M23] = mulAndAdd(row3, col2);
                    ret[M33] = mulAndAdd(row3, col3);
#endif
                }
                else
                {
            ...
            static Matrix& translate(Matrix& m, const Vector3& vector)
            {
                if(Utils::SIMD::ready)
                {
                    float32x4_t r = loadRow(m, 3);
                    float32x4_t v = vld1q_f32(vector.data());
                    for (UINT i = 0; i < 4; ++i) {
                        float32x4_t c = load(m, i, i+4, i+8);
#if defined(__aarch64__)
                        m[12+i] += vaddvq_f32(vmulq_f32(c, v));
#else
                        float d[4];
                        vst1q_f32(d, vmulq_f32(c, v));
                        m[12+i] += d[0] + d[1] + d[2];
#endif
                    }
                    return m;
