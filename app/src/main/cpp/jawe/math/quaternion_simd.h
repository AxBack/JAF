...
#define W 3
        ...
        public:
            ...
            virtual Quaternion operator*(const float rhs) const
            {
                ...
                {
                    Quaternion v;
                    vst1q_f32(v.m_data, vmulq_n_f32(vld1q_f32(m_data), rhs));
                    return v;
                }
                return {m_data[X] * rhs,
                        m_data[Y] * rhs,
            ...
            virtual Quaternion operator/(const float rhs) const
            {
                if(Utils::SIMD::ready)
                {
                    ...
                    vst1q_f32(v.m_data, vdivq_f32(vld1q_f32(m_data), vdupq_n_f32(rhs)));
#else
                    vst1q_f32(v.m_data, vmulq_f32(vld1q_f32(m_data), vrecpeq_f32(vdupq_n_f32(rhs))));
#endif
                    return v;
                }
                return {m_data[X] / rhs,
            ...
            static float dot(const Quaternion& q1, const Quaternion& q2)
            {
                ...
                {
#if defined(__aarch64__)
                    return vaddvq_f32(vmulq_f32(vld1q_f32(q1.m_data), vld1q_f32(q2.m_data)));
#else
                    float d[4];
                    vst1q_f32(d, vmulq_f32(vld1q_f32(q1.m_data), vld1q_f32(q2.m_data)));
                    return d[0] + d[1] + d[2] + d[3];
#endif
                }
